...
def get_spolka():
    ...
    return json.dumps(Kursy.query.filter(Kursy.sp_id == Spolka.query.filter_by(nazwa=nazwa).first().id).all(),
                      cls=AlchemyEncoder)
...
def get_spolka_by_filter():
    ...
    liczba_trans_max = liczba_trans_max_value if request.args.get('liczba_trans_max') is None else request.args.get(
        'liczba_trans_max')
    liczba_trans_min = liczba_trans_min_value if request.args.get('liczba_trans_min') is None else request.args.get(
        'liczba_trans_min')
...
def get_spolka_by_filter_analiza():
    ...
    roi = round((((late_invest - early_invest) / (float(inwestycja) * kurs_od) * 100)),3)
    ...
    return json.dumps({"roi": str(roi) + '%', "spolka": spolka_nazywa, "kurs_max": highest_kurs_value.get('kurs'),
                       'data_dodania_max': highest_kurs_value.get('data_dodania').strftime('%m/%d/%Y')},
                      cls=AlchemyEncoder)
...
def add_kurs():
    ...
    if Spolka.query.filter_by(nazwa=nazwa).first():
        ...
        nowy_kurs = Kursy(data_dodania, kurs_otw, kurs_max, kurs_min, kurs_zamkn, liczba_trans, wolumen, obrot,
                          spolka.id)
...
def wyszukiwanie_data():
    ...
    from app.models.models import Spolka, Kursy
    from app.models.models import AlchemyEncoder
    return json.dumps(Kursy.query.filter(Kursy.sp_id == Spolka.query.filter_by(nazwa=spolka_a).first().id).filter(
        Kursy.data_dodania >= datetime.date.fromisoformat(from_date)).all(), cls=AlchemyEncoder)
